...
esp_err_t enroll_new_face(uint8_t *image_buffer, size_t image_len, int width, int height,
    ...
    // Face is at 0 of both x,y
    int adjusted_face_x = 0;
    int adjusted_face_y = 0; 
    // face_w and face_h are ok
    ...
    std::vector<float>* new_face_embedding = enroller_recognizer_client.extract_embedding_from_cropped_box(
        image_buffer, width, height, // dimensions of the received image
        adjusted_face_x, adjusted_face_y, face_w, face_h, // box and dimensionsalligned with the cropped image
        adjusted_keypoints);

    if (!new_face_embedding) {
        ESP_LOGE(TAG, "Embedding failed. No face detected or failed extraction.");
        return ESP_FAIL;
    }
    ...
    if (database_add_face(&new_face_meta) == ESP_OK) {
        ESP_LOGI(TAG, "**********************************************");
        ESP_LOGI(TAG, "* RESULT: NEW FACE ENROLLED! ID: %d (%s) *", new_face_meta.id, new_face_meta.name);
        ESP_LOGI(TAG, "**********************************************");
        return ESP_OK;
    } else {
