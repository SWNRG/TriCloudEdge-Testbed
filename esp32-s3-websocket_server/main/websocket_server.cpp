 ...
 *
 * the only job that the websocket server does, is upon receipt of an imnage, pass it
 * to the image_processor module, and free the memory to be ready for the next one.
 * Dont insert other intelligence here, the websocket server has to remain agnostic
 */
...
static const char* TAG = "WEBSOCK_SRVR";
#define MAX_WEBSOCKET_CLIENTS CONFIG_LWIP_MAX_ACTIVE_TCP

//includes image dimensions because the cropped image from client is variable
typedef struct {
    uint8_t *buffer;
    size_t total_size;
...
static void reset_client_frame_state(int fd) {
    ...
    if (client_index != -1) {
        if(client_frame_states[client_index].buffer) {
            ...
            ESP_LOGI(TAG, "Freeing frame buffer for client %d", fd);
#endif
            free(client_frame_states[client_index].buffer);
#if EXTRA_HEAP_LOGGING
            ESP_LOGI(TAG, "Free heap after buffer free (reset_state): %" PRIu32, (uint32_t)heap_caps_get_free_size(MALLOC_CAP_INTERNAL | MALLOC_CAP_8BIT));
#endif       
        }
...
#if EXTRA_HEAP_LOGGING
        ESP_LOGI(TAG, "Client frame state reset for fd %d", fd);
#endif
    }
}
    ...
    if (ws_pkt.type == HTTPD_WS_TYPE_TEXT) {
            ...
            if (cJSON_IsString(type)) {
                ...
                if (strcmp(type->valuestring, "heartbeat") == 0) {
                    ...
                    if (client_frame_states[client_index].is_receiving) {
                         ESP_LOGW(TAG, "Received frame_start while already receiving for fd %d. Resetting state.", httpd_req_to_sockfd(req));
                    ...
                    if (cJSON_IsNumber(size) && cJSON_IsNumber(id) && cJSON_IsNumber(width) && cJSON_IsNumber(height) &&
                        ...
                        if (client_frame_states[client_index].buffer) {
                            ...
                            ESP_LOGI(TAG, "Got frame_start for ID: %u, Size: %d, Dimensions: %dx%d, Box: [%d,%d,%d,%d], Keypoints size: %zu",
                                     (unsigned int)id->valueint, (int)size->valueint, (int)width->valueint, (int)height->valueint,
...
#if EXTRA_HEAP_LOGGING
                            ESP_LOGI(TAG, "Free heap after buffer alloc: %" PRIu32, (uint32_t)heap_caps_get_free_size(MALLOC_CAP_INTERNAL | MALLOC_CAP_8BIT));
#endif
                     ...
                     if (client_frame_states[client_index].is_receiving) {
                        if (client_frame_states[client_index].received_size == client_frame_states[client_index].total_size) {
                            ESP_LOGI(TAG, "Transfer complete for Frame ID: %u. Total size: %d",
                                (unsigned int)client_frame_states[client_index].id, (int)client_frame_states[client_index].total_size);

#if EXTRA_HEAP_LOGGING
                            ESP_LOGI(TAG, "Ready to call image_processor_handle_new_image with:");
                            ESP_LOGI(TAG, "  Buffer Addr: %p", client_frame_states[client_index].buffer);
                            ESP_LOGI(TAG, "  Buffer Len: %zu", client_frame_states[client_index].total_size);
                            ESP_LOGI(TAG, "  Cropped Img Dims (Width x Height): %d x %d",
                                client_frame_states[client_index].width, client_frame_states[client_index].height);
                            ESP_LOGI(TAG, "  Original Face Box (x,y,w,h): %d,%d,%d,%d",
                                client_frame_states[client_index].face_x, client_frame_states[client_index].face_y,
                                client_frame_states[client_index].face_w, client_frame_states[client_index].face_h);
                            ESP_LOGI(TAG, "  Keypoints Count: %zu", client_frame_states[client_index].keypoints.size());
                            ESP_LOGI(TAG, "  Free heap before image_processor: %" PRIu32 "", (uint32_t)heap_caps_get_free_size(MALLOC_CAP_INTERNAL | MALLOC_CAP_8BIT));
#endif
                            /* Pass the image to image_processor. NO OTHER BUSINESS HERE */
        ...
        if (client_frame_states[client_index].is_receiving) {
            if (ws_pkt.len > 0 && (client_frame_states[client_index].received_size + ws_pkt.len <= client_frame_states[client_index].total_size)) {
                // Read the actual binary payload into the pre-allocated buffer
        ...
        ESP_LOGI(TAG, "Received WebSocket frame type %d from fd %d", ws_pkt.type, httpd_req_to_sockfd(req));
...
esp_err_t start_websocket_server(void) {
    ...
    ESP_LOGI(TAG, "Websocket server on, port: '%d', stack size %d, recv_timeout %d, send_timeout %d",
    ...
    ESP_LOGI(TAG, "WebSocket server up & running!");
    return ESP_OK;
}
