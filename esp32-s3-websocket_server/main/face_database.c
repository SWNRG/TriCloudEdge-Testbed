...
esp_err_t database_init(void) {
    ...
    if (!root || !cJSON_IsArray(root)) {
        ...
        cJSON_Delete(root);
        // Attempt to reset to an empty array
        storage_write_file(METADATA_PATH, "[]");
        // Re-attempt read (recursive call, but safe)
...
void database_deinit(void) {
    ...
    s_db.count = 0;
    ESP_LOGI(TAG, "Database deinitialized. Memory freed."); // Added log
}

esp_err_t database_get_all_faces(face_record_t** out_faces, int* out_count) {
    // Ensure database is initialized before returning records
